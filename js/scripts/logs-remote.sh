...
format "## Recent Sync missing for items (last ${LAST_LINES} entries per host)"
for host in "${HOSTS[@]}"; do
    format "### Host: ${host}"
    ssh -i "$SSH_KEY" "$host" "docker logs js-scrape-1 2>&1 | grep 'Sync missing' | tail -${LAST_LINES}" | \
    grep --color=always 'missingIn\(Local\|Remote\)=[0-9]*'
done
echo
